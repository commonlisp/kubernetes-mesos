...
func Test_parseMesosState(t *testing.T) {
	...
	assert(t, "parseMesosState does not yield an error", err == nil)
	assert(t, "parseMesosState yields a non-nil state", state != nil)
	assert(t, "parseMesosState yields a state with 3 nodes", len(state.nodes) == 3)
}
...
func Test_listSlaves(t *testing.T) {
	...
	assert(t, "createMesosClient does not yield an error", err == nil)

	slaveNodes, err := mesosClient.listSlaves(context.TODO())

	assert(t, "listSlaves does not yield an error", err == nil)
	assert(t, "listSlaves yields a collection of size 3", len(slaveNodes) == 3)
	...
	assert(
		t,
		"listSlaves yields a collection with the expected hostnames",
		reflect.DeepEqual(expectedHostnames, actualHostnames))
}
